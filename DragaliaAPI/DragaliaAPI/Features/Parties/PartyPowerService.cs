...
) : IPartyPowerService
{
    ...
    )
    {
        ...
        {
            power += await this.CalculateCharacterPower(partySetting, isFirst, bonusList);
            isFirst = false;
        }
    ...
    public async Task<int> CalculatePartyPower(DbParty party, FortBonusList? bonusList = null)
    {
        ...
        {
            power += await this.CalculateCharacterPower(partyUnit, isFirst, bonusList);
            isFirst = false;
        }
    ...
    {
        return await this.CalculateCharacterPower(
    ...
    )
    {
        ...
        abilityIdList.AddRange(
            crests.SelectMany(x =>
                MasterAsset.AbilityCrest[x.AbilityCrestId].GetAbilities(x.AbilityLevel)
            )
        );
