...
public class V17UpdateTest : SavefileUpdateTestFixture
{
    ...
    {
        await this.AddToDatabase(new DbPlayerDragonData() { DragonId = DragonId.Arsene });

        this.ApiContext.PlayerDragonReliability.AsNoTracking()
            .Should()
            .NotContain(x => x.ViewerId == this.ViewerId && x.DragonId == DragonId.Arsene);
        ...
        this.ApiContext.PlayerDragonReliability.AsNoTracking()
            .Should()
            .Contain(x =>
                x.ViewerId == this.ViewerId && x.DragonId == DragonId.Arsene && x.Level == 30
            );
    }
}
